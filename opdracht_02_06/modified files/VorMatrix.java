...
public class VorMatrix extends java.util.Observable{
    ...
    /**
     * Test if 4 adjecent would be achieved after a specific move
     * @param column the collum the move would be made on
     ...
     * @post returns wether the move would end the game
     * @author Jeroen Weltens
     */
    public boolean test_move(VierOpEenRij.FILL color, int column) throws ColumnFullException{
        int row = 0;
        while($vorMatrix[column][row] != VierOpEenRij.FILL.EMPTY && row < VierOpEenRij.ROWS - 1)
            row++;
        if($vorMatrix[column][row] != VierOpEenRij.FILL.EMPTY)
            throw new ColumnFullException(column);
        $vorMatrix[column][row] = color;
        boolean result = fourAdjacent(color);
        $vorMatrix[column][row] = VierOpEenRij.FILL.EMPTY;
        return result;
    }
